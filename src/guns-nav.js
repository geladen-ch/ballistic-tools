...
// the app is currently inside it, and where "Done" (or Arsenal's own "Set
// active") should send the user back to. Both ephemeral (page-lifetime
// only, not persisted) — the return path specifically mirrors
// arsenal-prefill.js's one-shot handoff pattern, and is deliberately
// separate from that module since it's not a form prefill. Also home to
// the source-aware routing (resolveGunsDestination/goToGuns) shared by
// every plain "go to Guns" entry point and guns-summary.js's "Change"
// button, so they all land on the same sub-tab for the same rifle.
