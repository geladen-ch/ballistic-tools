...
There is one honest accident preserved from the original: the stopping condition is checked *after* the splice, so the loop can and typically does remove one point past the floor, bottoming out at nine rather than ten. It is kept because by the time the glitch was noticed, the port was validated against real tracks corpus as a whole, and changing it would invalidate that validation for no known gain. Also, there is no particular justification behind the number 10; it was chosen as a ‘reasonable floor, which still makes sense’, so why not 9.
