...
Zeroing a rifle, at its core, means dialing the sight to whatever angle sends the bullet to the point of aim at one specific distance — the zero range. Every other sight correction, at every other range, is then computed relative to that same zero angle. That angle naturally depends on the specific cartridge and bullet involved: the faster a bullet reaches the zero range, the smaller the angle needs to be. If you then fire a different cartridge without changing anything about the zero, its own trajectory calculations come out wrong — exactly to the degree the two cartridges' ballistics differ at the zero range. The classic case: you zero the rifle with cheap surplus or practice ammunition, then load a premium hunting or duty cartridge whose actual point of impact, at any given range, is offset from where its own independent zero would have put it.
