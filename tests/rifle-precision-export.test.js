...
test('resolveImportItem with no conflict, no id, mints a fresh id', () => {
  ...
  assert.deepEqual(result, { action: 'save', record: { name: 'New One', id: 'rp-project-generated' } });
});

test('resolveImportItem matches an existing project by id even when names differ', () => {
...
test('resolveImportItem falls back to name matching when the imported id does not resolve locally', () => {
  ...
  const result = resolveImportItem(
    { id: 'other-id', name: 'My Project', modifiedAt: '2021-01-01T00:00:00.000Z' },
    { existingList: [existing], mode: 'overwrite', generateId: genId('rp-project'), nameTaken: () => false }
  );
  assert.deepEqual(result, {
    action: 'save',
    record: { id: 'existing-id', name: 'My Project', modifiedAt: '2021-01-01T00:00:00.000Z' }
  });
});
